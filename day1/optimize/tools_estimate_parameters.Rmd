...
```{r, message = FALSE}
data.file <- here("day1/data/fig4a.csv")
...
wss <- function(dv, pred, par=NULL) {
  sum(((dv-pred)/dv)^2) ## dnorm(yobs, yhat, sd, log = TRUE)
}
```
...
fit3 <- GenSA(
  NULL, pred, lower+0.5, upper-0.5, .data = data, yobs = yobs, 
  control = sacontrol
)
```
...
# 
# fit4 <- hydroPSO(
#   theta, fn = "pred", lower = lower + 0.5, upper = upper - 0.5, 
...
```{r}
results <- list(theta, fit1$par, fit1b$par, fit2$optim$bestmem, fit3$par)

results <- map(results, exp)

tibble(
  method = c("initial", "newuoa", "nelder", "DEoptim", "SA"),
