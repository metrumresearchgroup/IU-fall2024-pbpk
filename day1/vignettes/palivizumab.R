...
mod <- mread("palivizumab.cpp", here("day1/model"))
...
ggplot(summ, aes(x= time)) + 
  ...
  ylab("Palvizumab concentration (mcg/mL)") + 
  scale_x_continuous(name="Time (hr)", breaks = seq(0,168,12))

#ggsave(filename="~/Desktop/palivizumab.png", width=6, height = 4)
...
df <- nest_join(d, data) %>% unnest(cols = c(data)) %>% arrange(ID,time)
...
summ <- 
  ...
  as_tibble() %>% 
  group_by(time) %>% 
  summarise(lo = quantile(DV,0.05), hi = quantile(DV,0.95),med = median(DV))
...
#ggsave(filename="~/Desktop/palivizumab_2.png", width=6, height = 4)
